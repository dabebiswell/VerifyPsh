function New-VerifyScript {
<#
.SYNOPSIS
    Generates a standalone, interactive PowerShell script to verify file integrity.

.DESCRIPTION
    New-VerifyScript creates a completely standalone PowerShell script that contains
    the expected cryptographic hash of a specified file. When run, the generated 
    script will recalculate the hash of the target file and compare it against the
    expected hash, providing interactive pass/fail output.

.PARAMETER TargetFile
    The path to the file you want to verify. If omitted, you will be prompted to
    provide it interactively.

.PARAMETER Algorithm
    The cryptographic hash algorithm to use. Valid options are SHA256, SHA384,
    SHA512, and MD5. Defaults to SHA256.

.PARAMETER Interactive
    Forces the script into interactive mode, prompting for the file path and algorithm
    even if parameters were not originally provided.

.EXAMPLE
    New-VerifyScript -TargetFile ".\my-archive.zip"
    Generates Verify-my-archive.ps1 using SHA256.

.EXAMPLE
    New-VerifyScript -TargetFile ".\my-archive.zip" -Algorithm SHA512
    Generates Verify-my-archive.ps1 using SHA512.
#>
    [CmdletBinding()]
    param (
...
Export-ModuleMember -Function New-VerifyScript
